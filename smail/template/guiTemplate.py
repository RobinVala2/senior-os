...
def getButtonConf():
    ...
    bg_active = JS.jsonRed('GUI_template', "buttons_selected")
...
class ApplicationFrameTemplate:
    def __init__(self, root: tkinter.Tk):
        ...
        bg_color = JS.jsonRed('GUI_template', "app_frame")
        self.root = root
...
class menuButtonCRT:
    ...
    def createOptArr(self):  # this reads values from conf.json and creates array based of length that was given
        _numberOfValues = JS.jsonRed('GUI_template', "num_of_menu_buttons")  # get values from conf.json
        _counter = 1
    ...
    def menuActionUp(self):  # this def goes up one menu button: menu_X -> menu_X+1
        ...
        if self.id_of_menu < len(self.option):
            ...
            for i in new_dict:
                new_dict[i].pack(side=LEFT, padx=JS.jsonRed('GUI_template', "padx_value"))
    ...
    def createButtons(self):  # this def creates menu and back action button
        ...
        font_family = JS.jsonRed('GlobalConfiguration', "fontFamily")
        font_size = JS.jsonRed('GlobalConfiguration', "fontSize")
        font_thickness = JS.jsonRed('GlobalConfiguration', "fontThickness")
        ...
        for i in self.option:
            ...
            self.button_dict[i]['font'] = font_info
...
class optionsButtonsCRT1:
    ...
    def createOptArr(self):  # this reads values from conf.json and creates array based of length that was given
        _numberOfValues = JS.jsonRed('GUI_template', "num_of_opt_on_frame")
        _counter = 1
        while _counter <= _numberOfValues:
            self.option.append(_counter)
            _counter += 1
    ...
    def createOPT1Buttons(self):
        ...
        bg = getValues[0]
        bg_active = getValues[1]
        numOPT = JS.jsonRed('GUI_template', "num_of_opt_on_frame")
        padxValue = JS.jsonRed('GUI_template', "padx_value")
        font_family = JS.jsonRed('GlobalConfiguration', "fontFamily")
        font_size = JS.jsonRed('GlobalConfiguration', "fontSize")
        font_thickness = JS.jsonRed('GlobalConfiguration', "fontThickness")
        font_info = font_family + " " + str(font_size) + " " + font_thickness
