...
## NVIDIA SPECIAL - hybrid GPUs support
    # ADDS 310 MB TO THE SYSTEM :))
...
sudo chroot "${HOME}/${work_folder}/chroot" apt update
sudo chroot "${HOME}/${work_folder}/chroot" apt-get intall --no-install-recommends nvidia-drivers
...
sudo cp "${file_dir}/"${pdf} "${HOME}/${work_folder}/chroot/root/PDF/"
...
EOF
	...
	### isolinux SPLASH IMG
sudo cp ${file_dir}/${splash} ${HOME}/${work_folder}/staging/isolinux/splash.png
...
sudo tee "${HOME}/${work_folder}/staging/boot/grub/theme.txt" << EOF
...
EOF
    ...
    ### GRUB splash IMG 
sudo cp ${file_dir}/${splash} ${HOME}/${work_folder}/staging/boot/grub/${splash}

# COPY GRUP DEPENDENCIES INTO EFI BOOT DIRECTORY
sudo cp "${HOME}/${work_folder}/staging/boot/grub/"{grub.cfg,theme.txt,${splash}} "${HOME}/${work_folder}/staging/EFI/BOOT/"
...
grub-mkstandalone -O x86_64-efi \
    ...
    "/boot/grub/grub.cfg=${HOME}/${work_folder}/tmp/grub-embed.cfg" \
    "/boot/grub/theme.txt=${HOME}/${work_folder}/staging/boot/grub/theme.txt" \
    "/boot/grub/${splash}=${HOME}/PERSIST/staging/boot/grub/${splash}"
