...
class MLdetectionOfSusURL:
    ...
    def machineLearning(self):
        generic = pd.read_csv(self.fullPathToCsv)
        ...
        print(generic.threat.value_counts(normalize=True).round(2))
        self.vectorizer = TfidfVectorizer()
        feature = self.vectorizer.fit_transform(generic.url)
        feature_train, feature_test, threat_train, threat_test = train_test_split(feature,
                                                                                  generic.threat,
                                                                                  random_state=0)
        print(
    ...
    def predictURL(self):
        print("-- detection --")
        url = ["https://www.google.en/"]
        vectorizer = pickle.load(open("vectorizer", "rb"))
        model = pickle.load(open("model", "rb"))
        feature = vectorizer.transform(url)
        predict = model.predict(feature)
        print(predict)
        print("throught predict")
        featureThrg = self.vectorizer.transform(url)
        predictThrg = self.model.predict(featureThrg)
        print("through predict:",predictThrg)
